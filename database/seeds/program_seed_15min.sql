-- Program Seed Data – 15-MINUTE PROGRAMS
-- Run AFTER migration 013_create_programs.sql
-- Safe to re-run: deletes existing 15-min programs first

DELETE pe FROM `Program_Exercise` pe
INNER JOIN `Program` p ON pe.program_id = p.id
WHERE p.duration_min = 15;
DELETE FROM `Program` WHERE duration_min = 15;
